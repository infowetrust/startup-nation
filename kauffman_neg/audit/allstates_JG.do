...
forvalues i = 1/`n'{
	...
	u `state'.nomatch_old.dta, clear
	destring equityvalue, replace force
	save `state'.nomatch_old.dta, replace
	
	u `state'.only.dta , replace
	keep if mergerdate != .
	destring equityvalue, replace force
	append using `state'.nomatch_old.dta
	gen is_match = dataid != ""
	...
	*/
	...
	u `state'.nomatch_Z.dta, clear
	destring equityvalue, replace force
	save `state'.nomatch_Z.dta, replace
	
	u `state'.only.dta , replace
	keep if mergerdate != .
	destring equityvalue, replace force
	append using `state'.nomatch_Z.dta
	gen is_match = dataid != ""
	collapse (max) equityvalue, by(targetname is_match)
	qui: sum equityvalue , detail 
	local tot `r(sum)'
