...
//keep if first & address_order != . //drop many obs in old minimal, why not leave the address blank but keep obs?
keep if first == 1
drop first
gen address = trim(itrim(addr1 + " " + addr2 + " " +addr3))

// add this to remove address in missing address_order
replace address = "" if address_order == .
replace zipcode = "" if address_order ==.
replace city = "" if address_order ==.
...
gen local_firm = inlist(jurisdiction, "MO", "DE") & inlist(state, "MO", "")
save MO.dta, replace
